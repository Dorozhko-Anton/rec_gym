...
class PrimEnv2(gym.Env):
    ...
    def _add_random_item(self):
        embedding = (np.random.randn(self.embedding_dimension)-0.5)*self.cluster_var
    ...
    def _evolve(self):
        ...
        if self.time % self.new_items_interval == 0:
            n_items_to_add = self.new_items_size * self.n_items
    ...
    def _get_possible_items(self):
        ...
        for k, item in self.items.items():
            if (self.time < item.use_until) or (item.id > self.outdated_item_number):
